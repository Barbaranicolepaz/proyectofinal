...
#==================================PRODUCTOS====================
    def ventana_lista_productos(self):
        ...
        #crear columnas
        columnas=("codigo","producto","costo","precio","stock")
    ...
    def guardar_productos(self):
        #validamos para que no queden vacios los campos "codigo","producto","costo","precio","stock"
        if (self.txt_codigo_nuevo_productos.get()=="" or self.txt_producto_nuevo_productos.get()=="" or self.txt_costo_nuevo_productos.get()==""or self.
         txt_precio_nuevo_productos.get()=="" or self.txt_stock_nuevo_productos.get()==""):
            messagebox.showwarning("Guardando Producto","Algun campo no es valido revisar")
            return
        #capturador de error
        try:
           
            float(self.txt_costo_nuevo_productos .get())
            float(self.txt_precio_nuevo_productos.get())
            int(self.txt_stock_nuevo_productos.get())
            #establecer la conexion
            miconexion=sqlite3.connect('ventas.db')
            #creamos el cursor
            micursor=miconexion.cursor()
            ...
            txt_precio_nuevo_productos.get(),self.txt_stock_nuevo_productos.get())
            #consultamos la base de datos 
    ...
    def modificar_productos(self):
        if (self.txt_codigo_modificar_productos.get()==""or self.txt_producto_modificar_productos.get()=="" or self.txt_costo_modificar_productos.get()==""or self.txt_precio_modificar_productos.get()=="" or self.txt_stock_modificar_productos.get()==""):
          messagebox.showwarning("Guardando Productos","Algun campo no es valido revisar")
          return
        #capturador de error
        try:
            float(self.txt_costo_modificar_productos.get())
            float(self.txt_precio_modificar_productos.get())
            int(self.txt_stock_modificar_productos.get())
            #establecer la conexion
            miconexion=sqlite3.connect('ventas.db')
            #creamos el cursor
            micursor=miconexion.cursor()

            datos_modificar_productos=(self.txt_producto_modificar_productos.get() ,self.txt_costo_modificar_productos.get(),self.txt_precio_modificar_productos.get(),self.txt_stock_modificar_productos.get())
            
            #consultamos la base de datos 
            micursor.execute("UPDATE productos SET producto=?,costo=?,precio=?,stock=? WHERE codigo="+self.txt_codigo_modificar_productos.get(),(datos_modificar_productos))
            messagebox.showinfo('Modificar Productos',"Productos Modificado Correctamente")
            miconexion.commit()
            self.val_mod_prod=self.tree_lista_productos.item(self.productos_seleccionado,text='',values=(self.txt_codigo_modificar_productos.get(),self.txt_producto_modificar_productos.get(), self.txt_costo_modificar_productos.get(),self.txt_precio_modificar_productos.get(), self.txt_stock_modificar_productos.get()))
    ...
    def ventana_lista_ventas(self):
        ...
        self.frame_lista_ventas.grid(row=0,column=1,sticky=NSEW)

        self.lblframe_botones_ventas=LabelFrame(self.frame_lista_ventas)
        self.lblframe_botones_ventas.grid(row=0,column=0,sticky=NSEW)

        btn_detalle_ventas=tp.Button(self.lblframe_botones_ventas,text='DETALLE',width=12,bootstyle="success")
        btn_detalle_ventas.grid(row=0,column=0,padx=5)
        
        btn_cantidad_ventas=tp.Button(self.lblframe_botones_ventas,text='CANTIDAD',width=12,bootstyle="warnig")
        btn_cantidad_ventas.grid(row=0,column=1,padx=5)

        btn_descuento_ventas=tp.Button(self.lblframe_botones_ventas,text='DESCUENTO',width=12,bootstyle="Primary")
        btn_descuento_ventas.grid(row=0,column=2,padx=4)

        btn_pagar_ventas=tp.Button(self.lblframe_botones_ventas,text='PAGAR',width=12,bootstyle="success")
        btn_pagar_ventas.grid(row=0,column=3,padx=4)

        btn_credito_ventas=tp.Button(self.lblframe_botones_ventas,text='CREDITO',width=12,bootstyle="warning")
        btn_credito_ventas.grid(row=0,column=4,padx=5)

        btn_eliminar_ventas=tp.Button(self.lblframe_botones_ventas,text='ELIMINAR',width=12,bootstyle="danger")
        btn_eliminar_ventas.grid(row=0,column=5,padx=5)

        # self.lblframe_busqueda_ventas=tp.LabelFrame(self.frame_lista_ventas)
        # self.lblframe_busqueda_ventas.grid(row=2,column=0,sticky=NSEW)
        ...
        self.lblframe_tree_ventas=tp.LabelFrame(self.frame_lista_ventas)
        ...
        self.tree_lista_ventas=tp.Treeview(self.lblframe_tree_ventas,column=columnas,
                                        height=32,show='headings',bootstyle='success')
        ...
        #Ancho de columnas 
        self.tree_lista_ventas.column("codigo",width=50)
        self.tree_lista_ventas.column("detalle",width=255)
        self.tree_lista_ventas.column("cantidad",width=50)
        self.tree_lista_ventas.column("descuento",width=100)
        self.tree_lista_ventas.column("costo",width=100)
        self.tree_lista_ventas.column("pagar",width=100)
        self.tree_lista_ventas.column("credito",width=50)
        self.tree_lista_ventas.column("subtotal",width=50)
        ...
        #CREAR BARRA DE DESPLAZAMIENTO
        tree_scroll_ventas=tp.Scrollbar(self.frame_lista_ventas,bootstyle='success-round')                           
        tree_scroll_ventas.grid(row=2,column=2,pady=10)
        #CONFIGURACION  DE BARRA 
        tree_scroll_ventas.config(command=self.tree_lista_ventas.yview)
        lblframe_total_detalle_ventas=tp.LabelFrame(self.frame_lista_ventas)
        lblframe_total_detalle_ventas.grid(row=3,column=0,sticky=NSEW)
        self.ent_total_detalle_ventas=Entry(lblframe_total_detalle_ventas,width=39,font=('calibri',24),justify=CENTER)
        self.ent_total_detalle_ventas.grid(row=0,column=0)   
    ...
    def busqueda_descripcion(self):
        ...
        self.btn_busqueda_descripcion=tp.Button(master=self.lblframe_busqueda_ventas,text='abc',bootstyle='success-outles',command=self.busqueda_codigo)
        self.btn_busqueda_descripcion.grid(row=0,column=0,padx=1)

        self.busqueda_ventas=ttk.Entry(self.lblframe_busqueda_ventas,font=14,width=61)
        self.busqueda_ventas.grid(row=0,column=1,padx=5,pady=5)
    def busqueda_codigo(self):
        self.lblframe_busqueda_ventas=LabelFrame(self.frame_lista_ventas)
        self.lblframe_busqueda_ventas.grid(row=1,column=0)

        self.btn_busqueda_codigo=tp.Button(master=self.lblframe_busqueda_ventas,text=' # ',bootstyle='success-outles',command=self.busqueda_descripcion)
        self.btn_busqueda_codigo.grid(row=0,column=0,padx=1)

        self.busqueda_codigo_ventas=ttk.Entry(self.lblframe_busqueda_ventas,font=14,width=61)
        self.busqueda_codigo_ventas.grid(row=0,column=1,padx=5,pady=5)
    def ventana_busqueda_detalle_venta(self):
        self.frame_busqueda_detalle_venta=ScrolledFrame(self,width=550,autohide=True)
        self.frame_busqueda_detalle_venta.grid(row=0,column=2,padx=1,sticky=NSEW)
    def buscar_productos_venta(self,event):
        #capturador de error
        try:
            #establecer la conexion
            miconexion=sqlite3.connect('ventas.db')
            #creamos el cursor
            micursor=miconexion.cursor()
            #limpiamos nuestro scrollframe
            for wid in self.frame_busqueda_detalle_venta.winfo_children():
                wid.destroy()
            #consultamos la base de datos 
            micursor.execute("SELECT * FROM productos WHERE producto LIKE ?",(self.ent_total_detalle_ventas.get()+'%',))
            #con esto traigo todos los registros y lo guardo en datos
            datos_productos_detalle_venta=micursor.fetchall()
            codigo_busqueda_detalle_venta=StringVar()
            contador=0
            filas=2
            #recorrer cada fila en el scrollframe
            for row in datos_productos_detalle_venta:
                radbuton=Radiobutton(self.frame_busqueda_detalle_venta,text=row[1]+'\n'+ str(row[2])+'\n \n'+ str(f'${row[3]:,.2f}') ,value=row[0],indicator=0,width=37,height=5)
